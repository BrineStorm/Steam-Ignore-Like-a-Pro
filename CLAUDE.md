...
│   │   ├── main.js              # Phase 2: curator-page "Add to ignore queue" button + droplist (thin UI/wiring layer over EnqueueService); login-gated (no button when logged out) + surface-gated (in popup mode the button stays in place but LOCKED — greyed `.ilap-locked` + not-allowed + `curator_locked_popup` tooltip naming the Ctrl+Alt+Shift+I escape; the dropdown is forced shut and a pick is refused at click time, so a menu left open across a live surface flip can't stage)
...
└── tests/
    ...
    │   ├── migrate.unit.spec.js      # Node unit: src/migrate.js onInstalled — install→'widget', update+absent key→'popup', present key untouched, other reasons no-op
