...
2.  src/stats.js                → window.ILAP.StatsLogic (Last-Ignored record shape: key names, 20-entry cap, increment/pushHistory/nextState; before utils.js, and in the SW's importScripts — the two worlds that WRITE the record. NOT in popup.html, which only reads those keys by name)
...
5.  src/gate.js                 → window.ILAP.IgnoreGate (aggregate ignore-rate governor; needs getSessionID from utils.js)
