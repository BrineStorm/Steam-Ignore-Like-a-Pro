...
Evaluated against the source as of August 9, 2026, covering the additions since the July snapshot: the Phase-3 service-worker drain (`background.js` — the queue runs with no Steam tab open), the un-ignore/undo path (`ignore-log.js`, `UndoService`, the popup undo applet), the manual-ignore deferral queue (`type:'mi'` jobs drained through the rate gate), the solo un-ignore gesture (the `type:'miundo'` twin job, its cancel path, and the injectable session seam the rate gate grew alongside it), and the shared one-shot toast + storage-boundary sanitizer.
...
| **D** — Dependency Inversion | 8.5/10 | The drainer and `EnqueueService` are assembled with injected `store`/`enumerator`/`gate` deps; the DQ controller binds the registry as an adapter; `window.ILAP.*` / `chrome.*` access lives in thin adapters, and the Manual-Ignore app now builds its whole set in one place (`buildAdapters`), so wiring is separated from composition rather than interleaved in a constructor. The rate gate no longer discovers its own host either: the single thing it needs to know about the session (`hasSession`, tri-state) is one seam with a default for the tab, which the service worker — a world that loads no `utils.js` and has no DOM — replaces outright through `IgnoreGate.configure()`. That replaced an implicit arrangement in which the worker assigned `window.ILAP.getSessionID` at boot to satisfy a module that walked a ladder of facade names to work out which world it was running in. Not yet pristine: `curator/main.js` free functions, and `popup_queue.js` / `popup_undo.js` still reach a couple of singletons directly. What remains is within-world singleton reach rather than a cross-world contract resolved by name — documented, contained debt. |
