...
**Important:** Manual Ignore no longer fires an ungated instant POST — a swipe badges the game optimistically and enqueues a `type:'mi'` job the **curator drainer** sends through the `IgnoreGate`, gate-paced like EQ/DQ/curator. The ignore POST still lands (now via the content-script drainer, a few hundred ms later), so every test intercepts `POST /recommended/ignorerecommendation/` with `context.route` and fulfills a fake `{success:1}`; assertions use `expect.poll(() => calls…)`, which tolerates the deferred timing. (Route interception is world-independent — the earlier `window.ILAP.apiIgnoreGame` stub lived in the isolated world, invisible to `page.evaluate`.) These E2E specs were not changed for the deferral (the POST/stat/badge outcomes are unchanged); confirm on a live logged-in run.
